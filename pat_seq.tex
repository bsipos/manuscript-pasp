...
\newcommand{\mul}{\ensuremath{\mathrm{\mu l}}} % Botond, look, all I had to do to fix the trailing spaces was to remove the stray space characters that preceded the final }
...
\\
\vspace{5mm}
\diff{\textbf{Keywords:} add some; include ``high-throughput''} \note{Notice the use of my own `diff' and `note' commands.  The former is used to highlight text changes that I want you to see; if you approve, then just delete the command and leave the text in black.  If you disapprove, propose an alternative and leave it in blue.  `note' is used to add some comment, like this; update with your response, or delete if the issue is resolved (just as we have done in the past with Google docs comments).}
